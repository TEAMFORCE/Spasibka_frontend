    ...
    <div class="transaction_info_and_buttons">
      <div class="user_main_info">
        <Avatar
          ...
          :userName="
            myUserID == transaction.sender?.sender_id
              ? transaction.recipient?.recipient_first_name
              : transaction.sender?.sender_first_name
          "
          :userSurname="
            myUserID == transaction.sender?.sender_id
              ? transaction.recipient?.recipient_surname
              : transaction.sender?.sender_surname
          "
          :userPhoto="
            myUserID == transaction.sender?.sender_id
              ? transaction.recipient?.recipient_photo
              : transaction.sender?.sender_photo
          "
        />
        <div class="text_and_status">
          <div class="contacts">
            ...
            <h1
              class="sender_telegram"
              v-if="transaction.transaction_class.id == 'I'"
            >
              {{ transaction.sender?.sender_first_name }}
            </h1>
      ...
      <div class="amount_and_button">
        <div class="cancel_transaction_and_transfer_amount">
          ...
          >
            <h1 class="transfer_amount_number">
              {{ transaction.amount }}
            </h1>
...
export default {
  ...
  methods: {
    ...
    openOtherProfile() {
      if (!this.transaction.is_anonymous) {
        this.$router.push(
          "/other_profile/" +
            (this.myUserID == this.transaction.sender_id
              ? this.transaction.recipient_id
              : this.transaction.sender_id)
        );
      }
    },
    getDisplayDate(data) {
...
.transfer_amount.expense {
  background-color: var(--minorColorErrorPrimary);
}
