    ...
    <div class="scale_creation_form general-white-bg">
      ...
      <div class="scale_creation_header">
        <h1 class="scale_creation_header_text general-contrast-color">
          {{ $t("organization.settings.scale_creation") }}
        </h1>
        <h1 class="scale_creation_header_text general-contrast-color">
...
export default {
  name: "ScaleCreationForm",
  emits: ["closeScaleCreation"],
  components: {
    Datepicker,
    vSelect,
    EditScalefactorIcon,
    ...
    async createScale() {
      try {
        ...
        for (let i = 0; i < this.scaleFactors.length; ++i) {
          let curObject = {};
          curObject.event_id = this.scaleFactors[i].id;
        ...
        this.$emit("closeScaleCreation");
        notify(this.$t("organization.settings.scale_got_created"), "success");
      } catch (err) {
        notify(
