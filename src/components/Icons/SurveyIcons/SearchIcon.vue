<template>
  <div>
    ...
    >
      <g clip-path="url(#clip0_25563_14186)">
        <path
          ...
          :fill="generalContrast"
          fill-opacity="0.85"
        />
      </g>
      <defs>
        <clipPath id="clip0_25563_14186">
          <rect width="14" height="14" :fill="generalWhite" />
        </clipPath>
      </defs>
    </svg>
  </div>
...
export default {
  ...
  computed: {
    generalContrast() {
      if (this.generalContrastColor) {
        return this.generalContrastColor;
      } else {
    ...
    generalWhite() {
      if (this.generalWhiteColor) {
        return this.generalWhiteColor;
      } else {
        return getComputedStyle(document.documentElement).getPropertyValue(
          "--generalColorWhite"
        );
