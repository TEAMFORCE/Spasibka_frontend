<template>
  <div class="challenge_templates_sections">
    <h1 class="sections_header_text general-dark-color">
      {{ $t("challenges.challenge_templates.filter_by_categories") }}
    </h1>
...
import ChallengeTemplatesTopCategories from "./ChallengeTemplatesSectionsElements/ChallengeTemplatesTopCategories.vue";
import { mapStores } from "pinia";
import { useStore as useChallengeTemplatesStore } from "@/store/ChallengeTemplatesStore/index";
import { API_URIS } from "@/constants/api";
import ChallengeTemplatesSubCategories from "./ChallengeTemplatesSectionsElements/ChallengeTemplatesSubCategories.vue";

export default {
  name: "ChallengeTemplatesSections",
  components: {
    ChallengeTemplatesTopCategories,
    ChallengeTemplatesSubCategories,
  },
  ...
  watch: {
    "challengeTemplatesStore.selectedFilter.scope": {
      handler() {
        this.getSections();
      },
      immediate: true,
  ...
  methods: {
    async getSections() {
      try {
        let params = {
          scope: this.challengeTemplatesStore.selectedFilter.scope,
        };
