  ...
  <div class="main_page_balance_wrapper">
    ...
    <div class="distribute_balance_block">
      ...
      <div class="distribute_balance_expire">
        ...
        />
        <p class="expire_text">
          <span v-if="currentPeriodExists">{{ $t("mainPage.expiredTo") }}</span>
          <span v-if="currentPeriodExists">{{ `${daysLeft} ` + `${getNoun(daysLeft)}` }}</span>
  ...
  methods: {
    ...
    async getCurrentPeriod(){
      try {
        ...
        this.currentPeriodExists = false;
        throw err;
      }
    },
  },
...
.distribute_balance_block {
  ...
  height: 131px;
  border-radius: 20px;
  background: linear-gradient(93.97deg, #52c0ff 6.49%, #634fe6 93.51%), #f15929;
  display: flex;
  justify-content: space-between;
  align-items: flex-start;
  gap: 6px;
