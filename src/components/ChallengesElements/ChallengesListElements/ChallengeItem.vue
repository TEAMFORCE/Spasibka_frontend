<template>
  ...
  >
    <div class="challenge_item_without_button">
      ...
      <div class="start_end_dates general-secondary-color">
  ...
  computed: {
    ...
    challengePhoto() {
      if (this.challenge?.photo) {
        ...
        return this.photoAddress(`${this.challenge.photos[0]}?thumbnail=1`);
      } else {
        return null;
      }
    },
    ...
    notActive() {
      return (
        this.challenge?.challenge_condition !== CHALLENGE_CONDITIONS.active
      );
    },
    isWaiting() {
      return (
        this.challenge?.challenge_condition === CHALLENGE_CONDITIONS.waitToStart
      );
    },
    challengeLastUpdated() {
      let formattedTime;
      ...
      if (this.isWaiting) {
        ...
        formattedTime = formatTimestamp(this.challenge?.end_at);
      }
      ...
      const statusString = this.isWaiting
