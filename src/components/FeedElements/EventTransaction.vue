<template>
  <div class="event_transaction">
    <div class="user_info">
      <Avatar
        :type="'feed'"
        :userID="feedItem.transactionRecipientID"
      ...
      <div class="user_name_and_transaction_date">
        ...
        <!-- <h1
          @click="openOtherProfile(feedItem.transactionRecipientID)"
          class="user_name general-contrast-color"
        >
          {{
            /*feedItem.recipientFirstName*/
            feedItem.transactionRecipientFirstName || ""
          }}
          {{ " " + feedItem.transactionRecipientSurname || "" }}
        </h1> -->
        <h1 class="transaction_date general-secondary-color">
          {{ getDisplayDate(feedItem.transactionUpdatedAt) }}
        </h1>
      </div>
    </div>
  </div>
...
export default {
  ...
  computed: {
    feedItemHeader() {
      if (this.feedItem.header){
        const inputStr = this.feedItem.header;
        var regex = /https?:\/\/[^\/]+(\/[^ ]*)/g;
