<template>
  <div class="single_choice">
    <div v-show="question?.opened" ref="singleChoiceEdition" class="single_choice_edition">
      <Loading
      ...
      <SlickList
        ...
        v-if="question.inner_object"
        v-model:list="question.inner_object"
...
import { useStore as useSurveyStore } from "@/store/SurveyStore";
...
export default {
  ...
  methods: {
    positionLoader() {
      const el = this.$refs.singleChoiceEdition;
    ...
    selectOption(index) {
      for (let i = 0; i < this.question.inner_object.length; ++i) {
        if (i === index) {
          this.question.inner_object[i].selected = true;
        } else {
          this.question.inner_object[i].selected = false;
        }
      }
    },
    setOldAnswers() {
    ...
    async onDragEnd(el) {
      ...
      try {
        let draggedElement =
          this.question.inner_object[el.oldIndex];
        await axios.patch(
          API_URIS.questionnariesAnswerSelect + draggedElement.id + "/",
          {
            ordering: el.newIndex + 1,
            answer: draggedElement.answer,
            questionnaire_block: draggedElement.questionnaire_block,
          }
        );
        this.setOldAnswers();
      } catch (err) {
