<template>
  <div class="comments">
    <div class="comments_list">
      <div class="comment" v-for="(item, index) in comments">
        ...
        <div class="comment_content">
          <button
            v-if="profileStore?.profileInfo?.id === item.user.id"
          ...
          <div class="comment_user_avatar_wrapper">
            <img
              v-if="item.user.avatar != null"
              @click="openOtherProfile(item.user.id)"
              class="comment_user_avatar"
              :src="photoAddress(item.user.avatar)"
            />
            <div v-else class="comment_user_avatar_placeholder">
              <div class="comment_user_name_contracted_wrapper">
                <h1 class="comment_user_name_contracted general-white-color">
                  {{ contractedUserName(item.user.name, item.user.surname) }}
                </h1>
              </div>
            </div>
          </div>
          ...
          <div class="comment_text_and_like_dislike_buttons">
            <div class="comment_text">
              ...
              <h1 class="comment_time general-secondary-color">
                {{ getDisplayDate(item.created, item.is_edited) }}
              </h1>
            </div>
          </div>
        </div>
    ...
    <button
      v-if="comments.length < challengeDetails.comments_amount"
      class="show_next_comments_button"
      @click="fetchComments(comments.length)"
    >
...
export default {
  ...
  methods: {
    ...
    async fetchComments(offset, limit = 3, update = false) {
      try {
        const response = await axios.post(API_URIS.getComments, {
        ...
        this.$emit("updateCommentsAmount");
        const receivedComments = response.data.comments;
        if (!update) this.comments.push(...receivedComments);
        else {
          for (let i = 0; i < receivedComments.length; ++i) {
            this.comments[i] = receivedComments[i];
          }
          this.comments.splice(
            receivedComments.length,
            this.comments - receivedComments.length
          );
        }
      } catch (err) {
        throw err;
    ...
    async sendComment() {
      ...
      if (!isEmpty) {
        try {
          await axios.post(API_URIS.createComment, {
            challenge_id: this.challengeDetails.id,
            text: this.commentInput,
          });
          this.commentInput = "";
          this.fetchComments(0, this.comments.length + 1, true);
        } catch (err) {
          throw err;
        }
      }
