<template>
  <div class="test_user_result">
    <div class="top_container_with_button">
      <button
        class="back_to_test tooltip_container brand-secondary-bg"
        :data-tooltip="$t('challenges.back')"
        @click="
          $router.push({
            path: `/test_management/${$route.params.id}`,
...
<script>
import { userListMock } from "@/components/TestElements/TestManagementParticipants/userListMock";
...
export default {
  ...
  methods: {
    getParticipants() {
      setTimeout(() => {
        this.participantsList = userListMock.filter((el) => el.session);
        this.userIndex = this.participantsList.findIndex(
          (el) => el.session.id == this.$route.query.session_id
        );
      }, 500);
    },
    async getTestDetailResult(sessionId) {
      if (sessionId) {
        this.isLoading = true;
...
.test_user_result::-webkit-scrollbar {
  display: none;
}
